<template>
  <div id="main">
    ...
    <a-modal
      v-model:open="open"
      title="Ingresar valores de las celdas"
      @ok="handleOk"
      @cancel="handleCancel"
      :maskClosable="false"
    >
      <a-textarea
        class="textarea_estilo"
        v-model:value="texto_formulario"
      ...
      <table>
          ...
          <tr v-for="(row, rowIndex) in tableData" :key="rowIndex">
            ...
            <td :class="isDisabled ? 'classreadonly' : ''">
              <a-input-group compact>
                <a-input-number
                  @focus="handleFocus(rowIndex, 0)"
                  :status="poner_activo_estilo(rowIndex, 0)"
                  v-model:value="tableData[rowIndex][0]"
                  :readOnly="isDisabled"
                  :controls="!isDisabled"
                  class="ainputnumber"
                />
                <a-input-number
                  @focus="handleFocus(rowIndex, 1)"
                  :status="poner_activo_estilo(rowIndex, 1)"
                  v-model:value="tableData[rowIndex][1]"
                  :readOnly="isDisabled"
                  :controls="!isDisabled"
                  class="ainputnumber"
                />
                <a-input-number
                  @focus="handleFocus(rowIndex, 2)"
                  :status="poner_activo_estilo(rowIndex, 2)"
                  v-model:value="tableData[rowIndex][2]"
                  :readOnly="isDisabled"
                  :controls="!isDisabled"
                  class="ainputnumber"
                />
                <a-input-number
                  @focus="handleFocus(rowIndex, 3)"
                  :status="poner_activo_estilo(rowIndex, 3)"
                  v-model:value="tableData[rowIndex][3]"
                  :readOnly="isDisabled"
                  :controls="!isDisabled"
                  class="ainputnumber"
                />
                <a-input-number
                  @focus="handleFocus(rowIndex, 4)"
                  :status="poner_activo_estilo(rowIndex, 4)"
                  v-model:value="tableData[rowIndex][4]"
                  :readOnly="isDisabled"
                  :controls="!isDisabled"
                  class="ainputnumber"
                />
                <a-input-number
                  @focus="handleFocus(rowIndex, 5)"
                  :status="poner_activo_estilo(rowIndex, 5)"
                  v-model:value="tableData[rowIndex][5]"
                  :readOnly="isDisabled"
                  :controls="!isDisabled"
                  class="ainputnumber"
                />
            ...
            <td>
              <a-button tabindex="-1" type="primary" class="btn-showresult">{{
                getSum(tableData[rowIndex])
              }}</a-button>
            </td>
          </tr>
        </tbody>
        <tfoot>
          <tr>
            <td></td>
            <td>
              <a-button tabindex="-1" @click="toggleDisabled()"
                >Bloquear</a-button
              >
              |
              <a-button
                ...
                type="primary"
                danger
                >ordenar celdas</a-button
              >
              |
              <a-button tabindex="-1" @click="intercambiarvalores()"
                >intercambiar valores</a-button
              >
              |

              <a-tag color="blue"
                >diferencia de: {{ diferenciadevalores }}</a-tag
              >
            </td>
            <td></td>
          </tr>
